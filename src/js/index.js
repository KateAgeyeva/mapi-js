...
const movieController= async ()=>{
    ...
    if(id){
        ...
        //DISPLAY THE MOVIE
        movieView.displayMovie(data.movie,data.like.checkMovieLikedSatatus(id));
        ...
        // console.log(data.movie);
            ...
            alert('Something went wrong fetching the movie from id');
        }
    }
};
...
EVENT LISTENERS
1. FORM SUBMIT
2. ARROW BTN LEFT AND RIGHT
3. INCREASE OR DECREASE TICKETS NUMBER
4. DELETE AND UPDATE CART
5. BROWSER REFRESH
*/
...
window.addEventListener('load',()=>{
    ...
    //DISPLAY LIKES FROM LOCAL STORAGE
    const likes = data.like.showLikes();
    likes.forEach(el=>{
        likeView.displayLikeModal(el);
    });
    ...
    //DISPLAY THE CART FROM LOCALSTORAGE
...
//EVENT LISTENER ON SUBMIT SEARCH FORM
...
//EVENT LISTENER ON MOVIE SLIDER LEFT AND RIGHT BTNS
...
//INCREASE OR DECREASE TICKETS NUMBER, ADD TICKETS TO CART, LIKE MOVIE
classNames.mainContent.addEventListener('click', e=>{
    if(e.target.matches('.main-content__right-social--add, .main-content__right-social--add *')){
        ...
        movieView.updateTicketNum(data.movie);
    }
